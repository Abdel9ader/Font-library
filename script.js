...
const arabicWebsites = [
    {
        name: "الجزيرة",
        icon: "newspaper",
        font: "طاجاوال (Tajawal)"
    },
    {
        name: "سوق.كوم (أمازون)",
        icon: "shopping-cart",
        font: "المراعي (Almarai)"
    },
    {
        name: "موضوع",
        icon: "book-reader",
    ...
    },
    {
        name: "مستقل (حسوب)",
        icon: "laptop-code",
        font: "نوتو سانس (Noto Sans Arabic)"
    },
    {
        name: "العربية.نت",
        icon: "tv",
...
function renderFonts() {
    ...
    setTimeout(() => {
        ...
        if (paginatedFonts.length === 0) {
            ...
            paginatedFonts.forEach((font, index) => {
                const fontCard = document.createElement('div');
                fontCard.className = `font-card ${font.category}`;
                fontCard.style.animationDelay = `${index * 0.05}s`;
                ...
                fontCard.innerHTML = `
                            ...
                            <div class="font-preview">
                                ...
                                <div class="font-usage" style="cursor: pointer;" onclick="copyToClipboard('${font.fontFamily}')" title="Click to copy">
...
function renderWebsites() {
    ...
    if (typeof arabicWebsites !== 'undefined') {
        const fragment = document.createDocumentFragment();
        arabicWebsites.forEach(website => {
            ...
            websiteCard.innerHTML = `
                    ...
                    `;
            fragment.appendChild(websiteCard);
        });
        container.appendChild(fragment);
    }
}
...
function switchLanguage(lang) {
    ...
    //=========> Update Quick Links direction
    const quickLinks = document.getElementById('quickLinks');
    if (quickLinks) {
        quickLinks.className = lang === 'ar' ? 'footer-links rtl' : 'footer-links ltr';
    }
...
window.copyToClipboard = function (text) {
    navigator.clipboard.writeText(`font-family: ${text};`).then(() => {
        showToast('تم نسخ كود CSS بنجاح', 'CSS code copied successfully');
    }).catch(err => {
...
document.getElementById('previewBtn').addEventListener('click', () => {
    updateFontPreviews();
    showToast('تم تطبيق النص على جميع الخطوط', 'Text applied to all fonts');
});
...
//=========> Set up Scroll Events and Intersection Observer
...
function showToast(messageAr, messageEn) {
    ...
    toast.innerHTML = `<i class="fas fa-check-circle"></i> <span>${currentLanguage === 'ar' ? messageAr : messageEn}</span>`;
